    ...
    class Solution {
        public int myAtoi(String s) {
            ...
            for (int i = 0; i < s.length(); i++) {
                if (s.charAt(i) <= 57 && s.charAt(i) >= 48) { // 数字范围
                    isNumberRegion = true;
                    if (divisor == 0) {
                        divisor = 1;
                    }
                    if (res == 0 && s.charAt(i) == 48) { // 首个数字是0
                        continue;
                    } else {
                        res = res * 10 + Character.getNumericValue(s.charAt(i));
                        if (divisor * res > Integer.MAX_VALUE) {
                            return Integer.MAX_VALUE;
                        } else if (divisor * res < Integer.MIN_VALUE) {
                            return Integer.MIN_VALUE;
                        }
                    }
                } else if (divisor == 0 && s.charAt(i) == 43 && !isNumberRegion) { // +
                    divisor = 1;
                } else if (divisor == 0 && s.charAt(i) == 45 && !isNumberRegion) { //-
                    divisor = -1;
                } else {
                    if (res == 0 && s.charAt(i) == '_') {
                        continue;
                    }
                    break;
                }
